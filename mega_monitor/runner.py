...
from .notifier import notify_discord, notify_error, notify_unavailable
...
async def monitor_folder(name: str, url: str, state_dir: Path):
    ...
    try:
        ...
        renamed = [
            (prev_map[h]["path"], curr_map[h]["path"])
            for h in curr_map
            if h in prev_map and curr_map[h]["path"] != prev_map[h]["path"]
        ]
        ...
        permanent = e.code in (-9, -11, -14, -16)  # ENOENT, EACCESS, EKEY, EBLOCKED
        logger.warning("%s — MEGA reports folder unavailable (%s): %s", name, e.code, e.message)
        if permanent:
            notify_unavailable(name, url, e.code, e.message)
        else:
            notify_error(name, e)
...
async def run_monitor():
    ...
    logger.info(
        "Starting monitor – %d folders, interval=%ds",
        len(links),
    ...
    try:
        while True:
            ...
            logger.info(
                "Run completed — sleeping for %d s", settings.check_interval_seconds
            )
